...
## 특수 섹션 (PDF 섹션 자동 분리)
아래는 PDF에서 별도로 처리된다.
- `새가족`: 속이 아니라 신분이다. 새가족 섹션은 출석 심볼 대신 **모임 회차 `1주`~`4주` 칸에 모임 날짜**를 적는다. 속을 배정받지 못한 채 4회를 넘기면 `4주` 칸이 가장 최근 날짜를 보인다.
...
- **비고**: 마지막 출석(본당 포함, 기록이 없으면 명단 등록일)에서 3개월 이상 6개월 미만 지난 사람을 성도 → 새가족 순으로 적는다. 새가족은 인도자가 있으면 `이름(인도자)`로 적는다. 속 격자에서는 빼지 않고, 군인은 제외한다. 기준일은 출력 기간의 마지막 주일이다.
