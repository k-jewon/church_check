...
**다른 망(외부) 폰**은 cloudflared 가 있으면 서버가 **부팅 시 자동으로 터널을 열고 QR을 만든다**. 콘솔에 찍힌 URL·QR로 접속하거나, **관리 → 폰 접속(QR)** 화면의 QR을 스캔한다. 배포판(`release/`)에는 cloudflared 가 동봉되지만, **소스에서 개발 실행할 때는 프로젝트 루트에 `cloudflared.exe`(macOS는 `cloudflared`)를 두어야** 터널이 열린다 — 서버가 실행 시점의 작업 폴더 옆에서 찾기 때문이다. 없으면 이 단계는 자동으로 건너뛴다.
