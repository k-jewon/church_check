...
- **모드 구분(색·배지)**: 화면이 어느 영역인지 상단 네비 바로 구별된다.
  - **입력 모드**(파란 네비 + `입력 모드` 배지): `입력`. 관리자로 로그인했다면 `관리자 모드로 →` 전환 링크가 추가로 보인다.
...
### 새가족 모임 (`입력` 화면 아래 카드)
- 진행 중인 새가족이 있으면 출석 입력 화면 아래에 카드로 뜬다. **예배 뒤 모임까지 참여한 사람**을 체크한다.
- 회차는 예배 출석과 **별개**다. 예배만 오고 모임에 빠지면 회차가 오르지 않는다.
- 오른쪽에 지금까지의 회차(`1주차`~`4주차`)가 보인다. 4회를 채우고도 속을 배정받지 못하면 계속 `4주차`다.

### 방문 기록 (`입력 → 방문 기록`)
- 그 주일에 온 방문자의 **이름만** 적는다. 연락처도 출석 시간도 묻지 않는다 — 왔던 사람을 기억해 두는 것이 목적이다.
- 이름을 모르면 `무명`처럼 적어도 된다. 같은 사람이 다시 와도 이력을 잇지 않고 그 주일에 새 줄이 생긴다.
- 잘못 적었으면 **삭제**.

### 새가족 등록 (`입력 → 새가족 등록`)
- 새가족은 **출석 체크 도중에도 생기므로** 현장에서 바로 등록한다. 이름·출생연도 + 연락처·성별·인도자·방문경로.
- 등록하면 곧바로 출석 체크 대상이 되고, `새가족 모임` 카드에 회차 칸이 생긴다.
- **이 화면에는 목록이 없다.** 방금 만난 사람에게 물어 받아 적는 화면이지 남의 개인정보를 훑는 화면이 아니다 — 열람은 관리자 전용이다.

### 관리 → 새가족 관리 (`/admin/newfamily`)
- **개인정보(연락처·성별·인도자·방문경로)를 목록으로 볼 수 있는 유일한 화면**이라 관리자 암호로만 열린다. 입력 팀은 등록만 할 수 있고 이 목록은 보지 못한다.
- `새가족 등록`: 이름·출생연도 + 연락처·성별·인도자·방문경로. 경로를 **기타**로 하면 상세 텍스트가 저장된다.
- `수정`: 이름·출생연도와 등록정보를 고친다. 이 폼에는 속·직분 칸이 없어 **저장이 승격으로 새지 않는다**(그래서 명단 관리의 성도 수정 화면은 새가족을 막아 둔다). 비운 칸은 비워진다.
- `성도로 승격`: 과정을 마친 새가족에게 **속·직분을 배정**한다. 속 배정 규칙은 명단 관리의 성도 추가와 같다(속장은 속을 비워 두면 이름에서 새 속이 만들어진다).
- 승격해도 **등록정보와 회차 기록은 남고**, 이미 찍힌 출석은 당시 신분인 `새가족`으로 남는다.
- `삭제`: 잘못 등록한 사람을 지운다. **등록정보·모임 회차·이미 찍힌 출석이 함께 사라지고 되돌릴 수 없다.** 오등록을 되돌리는 용도이며, 더 나오지 않는 사람은 지우지 말고 그대로 둔다(3개월이 지나면 격자에서 빠져 비고로 간다). 성도는 이 경로로 지워지지 않는다.
