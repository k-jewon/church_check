...
Before creating more state, reuse task-owned state when safe, avoid duplicate artifacts, prefer the smallest mission/output that satisfies the task, use bounded retention, and stop creating more temporary state when growth or ownership becomes surprising.
